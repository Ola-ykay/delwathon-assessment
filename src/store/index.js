...
const index = createStore({
  ...
  mutations: {
    ...
    setLGAs(state, selectedState) {
      state.lgas = selectedState;
    },
  ...
  actions: {
    fetchSessions({ commit, state }) {
      ...
      return axios
        .get("https://eduthon-api.delwathon.com/api/academics/sessions")
        .then((response) => {
    ...
    async fetchClasses({ commit, state }) {
      try {
        ...
        const response = await axios.get(
          `https://eduthon-api.delwathon.com/api/academics/filter/branch_classes/${state.selectedBranch}`,
          {
            headers: {
        ...
        commit("setClasses", response.data.branch_classes);
      } catch (error) {
    ...
    fetchSections({ commit }, classId) {
      return axios
        .get(
          `https://eduthon-api.delwathon.com/api/academics/filter/class_sections/${classId}`
        )
        .then((response) => {
          commit("setSections", response.data.branch_class_sections);
        })
        .catch((error) => {
    ...
    fetchDepartments({ commit, state }) {
      return axios
        .get(
          `https://eduthon-api.delwathon.com/api/academics/filter/branch_departments/${state.selectedBranch}`
        )
        .then((response) => {
          commit("setDepartments", response.data.branch_departments);
        })
        .catch((error) => {
    ...
    fetchStates({ commit, state }) {
      ...
      return axios
        .get("https://eduthon-api.delwathon.com/api/states")
        .then((response) => {
    ...
    fetchLGAs({ commit, state }) {
      ...
      return axios
        .get(
          `https://eduthon-api.delwathon.com/api/lg_for_a_state/${state.selectedState}`
        )
        .then((response) => {
          commit("setLGAs", response.data.local_governments);
        })
        .catch((error) => {
